...
**Your mission:** Create an automated pipeline that runs daily. It must read all the event data, enrich it with data from the database, and produce a final CSV report summarizing sales performance by product category and customer segment.
...
* **Event JSON Format:**
    ```json
    [
      {"timestamp": "...", "customer_id": "c123", "event_type": "view_product", "product_id": "p789"},
      {"timestamp": "...", "customer_id": "c456", "event_type": "purchase", "product_id": "p101", "quantity": 2}
    ]
    ```
...
## 🛠️ Setup: How to Get Started

Follow these steps to set up your environment.

### Step 1: Set up the Python Environment

It looks like you're using `uv`. Let's create a virtual environment and install the dependencies.
...
2. **Extract (DB):** Create a class that connects to the PostgreSQL database (using `sqlalchemy`) and loads the `customers` and `products` tables into two separate DataFrames (hint: use `pd.read_sql`).
3. **Transform:**
